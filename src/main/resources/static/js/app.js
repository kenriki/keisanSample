		...
		// ------------------------------------------------------------------
		// ユーティリティ関数 (generateMathHTML - 桁揃えHTML生成) 
		// ------------------------------------------------------------------
		// (加算・減算・乗算の筆算表示で使用)
		...
		// (showAdditionProcess, showSubtractionProcess, showMultiplicationProcess は省略せずに維持)
		...
		function showMultiplicationProcess(rowId, x, y) {
			...
			for (let i = 0; i < maxLenY; i++) {
				...
				// 部分積の桁をずらす (iは0から始まるため、i個の '0' を追加)
		...
		function showDivisionProcess(rowId, x, y) {
			...
			// 割り算の主要部分の桁数
			const mainCols = strX.length + 1; // 割られる数 + 1桁の余白

			let stepsHTML = '';
			let currentRemainder = 0; // 現在の余り（引き算の結果）
			let dividendIndex = 0;    // 割られる数(strX)の現在見ている桁のインデックス
			let quotientIndex = 0;    // 商(strQ)の現在埋めた桁数

			// 商の桁数分、主要なステップを繰り返す
			while (quotientIndex < qLen) {

				// 1. 割られる数の新しい部分 (現在の余り + 新しく引き下ろした桁) を作る
				let workingDividend = currentRemainder * 10;

				// 商の桁を決定するために、割られる数から新しい桁を引き下ろす
				while (workingDividend < initialY && dividendIndex < strX.length) {
					workingDividend += parseInt(strX[dividendIndex], 10);
					dividendIndex++;
				}

				// 商の桁を計算 (商が0で、かつまだ先頭でない場合は次のループで桁を引き下ろす)
				...
				// 3. 引き算の結果 (currentRemainder) を計算
				currentRemainder = workingDividend - product;

				// --- HTML ステップ生成 ---

				// このステップの引き算の対象となる割られる数の部分の開始桁を決定
				// 商が確定した桁数から逆算して、割られる数の長さとの差分を求める

				const currentStepLength = (dividendIndex - 1);
				const productStr = String(product);

				// A. 引く数 (Product) の行
				// 商の桁 (quotientIndex) に対応する割られる数 (strX) の桁の位置に合わせる

				// 割られる数の全体を基準とした、この Product の開始インデックス
				...
				// C. 引き算の結果 (currentRemainder) + 新しく引き下ろす次の桁
				...
				workingDividend = currentRemainder; // 次のステップのために余りを設定
			}
		...
		operatorButtons.forEach(button => {
			button.addEventListener('click', function () {
				...
				if (activeOperatorDisplay) {
					const displayChar = selectedOperator === '*' ? '×' :
